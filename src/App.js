...
// Literature Content (Expanded 5x)
const literatureContent = [
  { 
    title: "1. 폐렴 (Pneumonia) - 심층 고찰", 
    content: `(1) 정의 및 역학: 폐렴은 세균, 바이러스, 진균 등 미생물 감염에 의해 폐실질(Parenchyma)에 급성 염증이 발생하는 질환이다. 지역사회 획득 폐렴(CAP)의 경우, 입원 환자의 사망률은 약 10~12%에 달하며, 중환자실 입원 시 30% 이상으로 증가한다. 본 대상자와 같은 ILD 기저질환자는 고위험군이다.\n\n(2) 병태생리 (Pathophysiology): 병원체가 기도를 통해 폐포에 도달하면, 숙주의 면역 반응으로 폐포 대식세포와 호중구가 활성화된다. 이 과정에서 IL-1, IL-6, TNF-a 등의 염증성 사이토카인이 방출되어 발열과 전신 증상을 유발한다. 또한, 폐포 모세혈관의 투과성 증가로 인해 혈장 성분과 혈구들이 폐포 내로 유출되어 삼출물(Exudate)을 형성한다. 이로 인해 가스 교환 면적이 감소하고 환기-관류 불균형(V/Q mismatch) 및 션트(Shunt) 효과가 발생하여 저산소혈증(Hypoxemia)을 초래한다.\n\n(3) 증상: 발열, 오한, 화농성 객담, 호흡곤란, 흉통이 주 증상이다. 특히 고령이나 면역저하자에서는 발열이 뚜렷하지 않을 수 있어 주의가 필요하다. 청진 시 수포음(Crackles)이나 천명음(Wheezing)이 들릴 수 있으며, 타진 시 탁음(Dullness)이 관찰된다.\n\n(4) 치료 가이드라인 (ATS/IDSA 2019): 기저질환이 있는 환자의 경우, 내성균 및 비정형균 감염 가능성을 고려하여 B-lactam(예: Ceftriaxone)과 Macrolide(예: Azithromycin)의 병용 요법 또는 Respiratory Fluoroquinolone 단독 요법을 권고한다.` 
  },
  { 
...
const VitalHistoryModal = ({ vital, data, onClose }) => {
    ...
    // Key Mapping for Modal Data
    const getKey = (label) => {
        if(label === 'BP') return 'sbp';
        if(label === 'HR') return 'hr';
        if(label === 'RR') return 'rr';
        if(label === 'SpO2') return 'spo2';
        if(label === 'BT') return 'bt';
        return 'sbp';
    };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-0 max-h-[60vh] overflow-y-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">{data.map((d, i) => <tr key={i}><td className="p-3">{d.time}</td><td className="p-3 font-bold text-[#005EB8]">{vital.label === 'BP' ? `${d.sbp}/${d.dbp}` : d[getKey(vital.label)]}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4 text-sm">
            ...
            {type === 'fall' ? (
             ...
             <div className={`mt-4 p-3 rounded border ${type === 'fall' ? 'bg-amber-50 border-amber-100 text-amber-800' : 'bg-teal-50 border-teal-100 text-teal-800'}`}>
                <strong>💡 중재:</strong> {type === 'fall' ? '낙상 고위험 표지판 부착, 침상 난간 올림, 보호자 교육' : '피부 상태 건조하게 유지, 2시간마다 체위 변경, 영양 공급'}
            </div>
        </div>
      </div>
    </div>
